...
# USE SWITCH CASE
    ...
    DROP_EXPORT_DUPLICATE_ROWS_ENDPOINT = f"{
        API_BASE_URL}/drop_export_duplicate_rows"
    ...
    st.markdown("<h1 style='text-align: center;'>DiscSim Module 4: Administrative Data Quality Checks</h1>",
                unsafe_allow_html=True)

    # File selection
    file_option = st.radio(
        "Choose an option:", ("Upload a new file", "Select a previously uploaded file"))
    ...
    if file_option == "Upload a new file":
        uploaded_file = st.file_uploader(
            "Choose a CSV file to begin analysis", type="csv")
            ...
            if "current_file_name" not in st.session_state or st.session_state.current_file_name != uploaded_file.name:
                ...
                if response.status_code == 200:
                    ...
                    file_response = requests.get(
                        f"{GET_FILE_ENDPOINT}/{file_id}")
                    if file_response.status_code == 200:
                        file_data = file_response.json()

                        # Extract file content and filename
                        file_content = file_data["content"].encode(
                            'utf-8')  # Convert back to bytes
                    ...
                    st.warning(
                        "A file with this name already exists. Please upload a different file.")
                    return  # Stop further processing
                else:
                    st.error("Failed to upload file.")
            else:
                st.info(
                    f"File '{uploaded_file.name}' has already been uploaded in this session.")
        ...
        if response.status_code == 200:
            ...
            selected_file = st.selectbox(
                "Select a previously uploaded file", file_names)

            if selected_file:
                try:
                    file_id = next(
                        file["id"] for file in files if file["filename"] == selected_file)
                    ...
                    if file_response.status_code == 200:
                        ...
                        file_content = bytes(file_data["content"], 'utf-8')
                    ...
                    st.error(f"No file found with the name '{
                             selected_file}'. Please try again.")
        else:
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Running preliminary tests on the uploaded file..."):
            try:
                ...
                files = {"file": ("uploaded_file.csv",
                                  uploaded_file, "text/csv")}
                response = requests.post(
                    PRELIMINARY_TESTS_ENDPOINT, files=files)
                    ...
                    if result["status"] == 0:
                        ...
                        if result["warnings"]:
                            ...
                            for warning in result["warnings"]:
                                ...
                                with col1:
                                    st.write(f"- {warning}")
                                with col2:
                                    relevant_func = get_relevant_functionality(
                                        warning)
                        ...
                        if functionality == "Unique ID Verifier":
                            ...
                            if st.button("Find Unique IDs"):
                                with st.spinner("Finding unique IDs..."):
                                    try:
                                        uploaded_file.seek(0)
                                        files = {
                                            "file": ("uploaded_file.csv", uploaded_file, "text/csv")}
                                        response = requests.post(
                                            FIND_UNIQUE_IDS_ENDPOINT, files=files)
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            if unique_ids:
                                                ...
                                                st.write(
                                                    "The best possible Column(s) or Combinations that can act as Unique ID are as follows:")
                                                df = pd.DataFrame(unique_ids)
                                                df['UniqueID'] = df['UniqueID'].apply(
                                                    lambda x: ' + '.join(x))
                                                df = df.rename(
                                                    columns={'UniqueID': 'Unique ID', 'Numeric_DataTypes': 'Is Numeric'})
                                                st.dataframe(
                                                    df, use_container_width=True)
                                            else:
                                                st.warning(
                                                    "No unique identifiers found. All columns or combinations have at least one duplicate.")
                                        else:
                                            st.error(
                            ...
                            columns = st.multiselect(
                                "Select columns to check", options=df.columns.tolist())

                            if columns and st.button("Check Unique ID"):
                                with st.spinner("Checking unique ID..."):
                                    df_clean = df.replace(
                                        [np.inf, -np.inf], np.nan).dropna()
                                    data = df_clean.where(pd.notnull(
                                        df_clean), None).to_dict('records')
                                    payload = {"data": data,
                                               "columns": columns}
                                    response = requests.post(
                                        UNIQUE_ID_CHECK_ENDPOINT, json=payload)
                                    ...
                                    if response.status_code == 200:
                                        ...
                                        if result[1]:
                                            ...
                                            st.warning(result[0])
                                            st.write(
                                                "Go to Unique ID Verifier to check for column uniqueness")
                                    else:
                            ...
                            with st.expander("ℹ️ Info"):
                                ...
                                - Valid input format: CSV file
                                """)
                            uid_col = st.multiselect(
                                "Select unique identifier column(s)", df.columns.tolist())
                            kept_row = st.selectbox("Which duplicate to keep: first(keeps the first occurrence), last(keeps the last occurrence), or none(removes all occurrences)", [
                                                    "first", "last", "none"])
                            export = st.checkbox(
                                "Export duplicates", value=True)
                            chunksize = st.number_input(
                                "Chunksize (optional, leave 0 for no chunking): Use only for very large datasets, set to 10000 to start with", min_value=0, step=1000)
                                ...
                                with st.spinner("Processing..."):
                                    try:
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            unique_df = pd.DataFrame(requests.get(
                                                f"{GET_DATAFRAME_ENDPOINT}?data_type=unique").json())
                                            duplicate_df = pd.DataFrame(requests.get(
                                                f"{GET_DATAFRAME_ENDPOINT}?data_type=duplicate").json())
                                            # Visualize the results
                                            total_rows = len(
                                                unique_df) + len(duplicate_df)
                                            unique_rows = len(unique_df)
                                            duplicate_rows = len(duplicate_df)

                                            fig = plot_pie_chart([f"Unique Rows ({unique_rows})", f"Duplicate Rows ({
                                                                 duplicate_rows})"], [unique_rows, duplicate_rows], "Dataset Composition")
                                            st.plotly_chart(fig)
                                            # Display dataframes
                                            ...
                                            try:
                                                ...
                                                st.error(
                                                    f"Error displaying unique rows: {str(e)}")
                                            ...
                                            if export:
                                                ...
                                                try:
                                                    ...
                                                    st.error(
                                                        f"Error displaying duplicate rows: {str(e)}")
                                        else:
                                            st.error(
                                                f"Error: {response.status_code} - {response.text}")
                                    except Exception as e:
                            ...
                            if st.session_state.get('drop_export_complete', False):
                                ...
                                unique_filename = st.text_input(
                                    "Enter filename for unique rows (without .csv)", value="unique_rows")
                                if st.button("Download Unique Rows"):
                                    download_url = f"{GET_PROCESSED_DATA_ENDPOINT}?data_type=unique&filename={
                                        unique_filename}.csv"
                                    st.markdown(f'<a href="{download_url}" download="{
                                                unique_filename}.csv">Click here to download unique rows</a>', unsafe_allow_html=True)
                                    st.warning(
                                        "Please consider uploading the newly downloaded deduplicated file for further analysis.")

                                # Download Duplicate Rows (if exported)
                                if export:
                                    duplicate_filename = st.text_input(
                                        "Enter filename for duplicate rows (without .csv)", value="duplicate_rows")
                                    if st.button("Download Duplicate Rows"):
                                        download_url = f"{GET_PROCESSED_DATA_ENDPOINT}?data_type=duplicate&filename={
                                            duplicate_filename}.csv"
                                        st.markdown(f'<a href="{download_url}" download="{
                                                    duplicate_filename}.csv">Click here to download duplicate rows</a>', unsafe_allow_html=True)
                                        st.warning(
                                            "Note that this is the CSV containing all the duplicate entries, download the unique deduplicated file for better analysis.")
                            else:
                            ...
                            column_to_analyze = st.selectbox(
                                "Select column to analyze for missing entries:", options=df.columns.tolist(), index=0)
                            group_by = st.selectbox("Group by (optional): Analyze missing entries within distinct categories of another column. This is useful if you want to understand how missing values are distributed across different groups.", options=[
                                                    "None"] + df.columns.tolist(), index=0)
                            filter_by_col = st.selectbox("Filter by column (optional): Focus on a specific subset of your data by selecting a specific value in another column. This is helpful when you want to analyze missing entries for a specific condition.", options=[
                                                         "None"] + df.columns.tolist(), index=0)

                            if filter_by_col != "None":
                                filter_by_value = st.selectbox(
                                    "Filter value", df[filter_by_col].unique().tolist())
                            else:
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            if result["grouped"]:
                                                ...
                                                fig = px.bar(data, x=group_by, y=['Missing', 'Present'],
                                                             title=f"Missing vs Present Entries by {
                                                                 group_by}",
                                                             labels={
                                                                 'value': 'Percentage', 'variable': 'Status'},
                                                             color_discrete_map={
                                                                 'Missing': 'red', 'Present': 'green'},
                                                             text='value')
                                                fig.update_layout(
                                                    barmode='relative', yaxis_title='Percentage')
                                                fig.update_traces(
                                                ...
                                                if percentage is not None:
                                                    ...
                                                    fig = plot_pie_chart(
                                                        labels, values, "Missing vs Present Entries (%)")
                                                    st.plotly_chart(fig)
                                                else:
                                                    st.write(f"Missing entries: {
                                                             count} (percentage unavailable)")

                                            if result["filtered"]:
                                                st.info(f"Results are filtered by {
                                            ...
                                            if "missing_entries_table" in result:
                                                if not result["missing_entries_table"]:
                                                    st.warning(
                                                        "The missing entries table is empty. There might be no missing entries, or an issue occurred.")
                                                else:
                                                    missing_entries_df = pd.DataFrame(
                                                        result["missing_entries_table"])

                                                    if column_to_analyze in missing_entries_df.columns:
                                                        missing_entries_df = missing_entries_df.sort_values(
                                                            column_to_analyze, ascending=False)
                                                        st.success(f"Sorted by column: '{
                                                                   column_to_analyze}'")
                                                    else:
                                                        st.warning(f"Column '{
                                                                   column_to_analyze}' not found in the missing entries table. Displaying unsorted data.")

                                                    st.write(
                                                        "Rows with Missing Entries:")
                                                    st.dataframe(
                                                        missing_entries_df, use_container_width=True)
                                            else:
                                                st.error(
                                                    "The 'missing_entries_table' key is not present in the API response.")
                            ...
                            column_to_analyze = st.selectbox(
                                "Select column to analyze", df.columns.tolist())
                            group_by = st.selectbox("Group by (optional): Analyze missing entries within distinct categories of another column. This is useful if you want to understand how missing values are distributed across different groups.", [
                                                    "None"] + df.columns.tolist())
                            filter_by_col = st.selectbox("Filter by (optional): Focus on a specific subset of your data by selecting a specific value in another column. This is helpful when you want to analyze missing entries for a specific condition.", [
                                                         "None"] + df.columns.tolist())
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            if result["grouped"]:
                                                ...
                                                grouped_df = pd.DataFrame(
                                                    grouped_data)
                                                grouped_df = grouped_df.sort_values(
                                                    "Zero Count", ascending=False)
                                                st.dataframe(
                                                    grouped_df, use_container_width=True)
                                                ...
                                                fig = px.bar(data, x=group_column_name, y=['Zero', 'Non-Zero'],
                                                             title=f"Zero vs Non-Zero Entries by {
                                                                 group_column_name}",
                                                             labels={
                                                                 'value': 'Percentage', 'variable': 'Entry Type'},
                                                ...
                                                analysis_df = pd.DataFrame(
                                                    [{"Zero Count": count, "Zero Percentage": f"{percentage:.2f}%"}])
                                                st.dataframe(
                                                    analysis_df, use_container_width=True)

                                                labels = ['Zero', 'Non-Zero']
                                                values = [
                                                    percentage, 100-percentage]
                                                fig = plot_pie_chart(
                                                    labels, values, "Zero vs Non-Zero Entries (%)")
                                                st.plotly_chart(fig)
                                            ...
                                            if "zero_entries_table" in result:
                                                zero_entries_df = pd.DataFrame(
                                                    result["zero_entries_table"])
                                                if column_to_analyze in zero_entries_df.columns:
                                                    zero_entries_df = zero_entries_df.sort_values(
                                                        column_to_analyze, ascending=False)
                                                else:
                                                    st.warning(f"Column '{
                                                               column_to_analyze}' not found in the zero entries table. Displaying unsorted data.")
                                                st.write(
                                                    "Rows with Zero Entries:")
                                                st.dataframe(
                                                    zero_entries_df, use_container_width=True)
                                        else:
                                            st.error(
                                                f"Error: {response.status_code} - {response.text}")
                                    except Exception as e:
                            ...
                            group_by = st.selectbox("Group by (optional)", [
                                                    "None"] + df.columns.tolist())
                            filter_by_col = st.selectbox("Filter by (optional)", [
                                                         "None"] + df.columns.tolist())
                            ...
                            if is_numeric_column(df[column_to_analyze]):
                                ...
                                with col1:
                                    operation = st.selectbox(
                                        "Operation", get_numeric_operations())
                                with col2:
                                    threshold = st.number_input(
                                        "Threshold", value=0.0, step=0.1)
                                invalid_condition = f"{operation} {threshold}"
                                include_zero_as_separate_category = st.checkbox(
                                    "Include zero entries as a separate category", value=True)
                            elif is_string_column(df[column_to_analyze]):
                                st.write(
                                    "Set condition for invalid string values:")
                                col1, col2 = st.columns(2)
                                with col1:
                                    operation = st.selectbox(
                                        "Operation", get_string_operations())
                                with col2:
                                    value = st.selectbox(
                                        "Value", df[column_to_analyze].unique().tolist())
                                invalid_condition = (operation, value)
                                include_zero_as_separate_category = False
                            elif is_datetime_column(df[column_to_analyze]):
                                st.write(
                                    "Set condition for invalid datetime values:")
                                col1, col2 = st.columns(2)
                                with col1:
                                ...
                                with col2:
                                    end_date = st.date_input(
                                        "End date(Inclusive)")
                                invalid_condition = (start_date.strftime(
                                    '%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
                                include_zero_as_separate_category = False
                            else:
                                st.write(
                                    "The column should include either numbers, strings, or dates.")
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            def display_detailed_data(data):
                                                if include_zero_as_separate_category:
                                                    for category in ["missing", "zero", "invalid", "valid"]:
                                                        if data[category]:
                                                            st.write(
                                                                f"{category.capitalize()} data (up to 10 rows):")
                                                            category_df = pd.DataFrame(
                                                                data[category])
                                                            st.dataframe(
                                                                category_df, use_container_width=True)
                                                        else:
                                                            st.write(
                                                                f"No {category} data found.")
                                                else:
                                            ...
                                            if result["grouped"]:
                                                ...
                                                combined_df = pd.concat(
                                                    all_groups_data, ignore_index=True)
                                                # Calculate percentages within each group
                                                combined_df['Percentage'] = combined_df.groupby(
                                                    'Group')['Count'].transform(lambda x: x / x.sum() * 100)
                                            ...
                                            if invalid_condition:
                                                st.info(f"Custom invalid condition applied: {
                                                        invalid_condition}")
                                        else:
                                            st.error(
                                                f"Error: {response.status_code} - {response.text}")
                            ...
                            if st.button("Generate Frequency Table"):
                                with st.spinner("Generating frequency table..."):
                                    try:
                                        ...
                                        response = requests.post(FREQUENCY_TABLE_ENDPOINT, files=files, data={
                                                                 "input_data": json.dumps(payload)})
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            if result["grouped"]:
                                                st.write(
                                                    "Combined Frequency Table for All Groups:")
                                                full_table, top_n_table = result["analysis"]

                                                # Determine if all values should be shown
                                                show_all_values = top_n <= 0 or top_n >= len(
                                                    full_table)

                                                if not show_all_values:
                                                    st.write(
                                                        f"Top {top_n} frequent values:")
                                                    top_n_df = pd.DataFrame(
                                                        top_n_table)
                                                    top_n_df = top_n_df.sort_values(
                                                        "count", ascending=False)
                                                    top_n_df = top_n_df[[
                                                        column_to_analyze, group_by, "count", "share %"]]
                                                    st.dataframe(
                                                        top_n_df, use_container_width=True)
                                                else:
                                                    st.write(
                                                    ...
                                                    full_df = full_df.sort_values(
                                                        "count", ascending=False)
                                                    st.dataframe(
                                                        full_df, use_container_width=True)
...
def pre_survey_analysis():
    ...
    def check_errors(params):
        response = requests.post(
            ERROR_HANDLING_ENDPOINT, json={"params": params})
        if response.status_code == 200:
            result = response.json()
    ...
    def l1_sample_size_calculator():
        ...
        # Input fields
        min_n_samples = st.number_input(
            "Minimum number of samples: The minimum number of data points that a supervisor will sample (typically 1). Range > 0", min_value=1, value=1)
        max_n_samples = st.number_input(
            "Maximum number of samples: The maximum number of data points that a supervisor can sample (if this is not high enough, the guarantee may not be possible and the algorithm will ask you to increase this). Range > min_n_samples", min_value=min_n_samples + 1, value=100)
        n_subs_per_block = st.number_input(
            "Number of subordinates per block: The number of subordinates that one supervisor will test. Range > 0", min_value=1, value=10)
        n_blocks_per_district = st.number_input(
            "Number of blocks per district", min_value=1, value=5)
        n_district = st.number_input(
            "Number of districts", min_value=1, value=1)
        level_test = st.selectbox(
            "Level of test", ["Block", "District", "State"])
        ...
        n_simulations = st.number_input(
            "Number of simulations: By default, this should be set to 100. The number of times the algorithm will be run to estimate the number of samples required. Higher n_simulations will give a more accurate answer, but will take longer to run. Range > 1", min_value=1, value=100)
        ...
        mean_disc = st.slider("Mean discrepancy score", min_value=min_disc,
                              max_value=max_disc, value=(min_disc + max_disc) / 2)
        std_disc = st.slider("Standard deviation of discrepancy score", min_value=0.0, max_value=(
            max_disc - min_disc) / 2, value=(max_disc - min_disc) / 4)
        distribution = st.selectbox(
            "Distribution: Distribution of discrepancy scores to be used for simulation. Currently, only uniform distribution is implemented. We will implement normal and other distributions in future versions.", ["uniform", "normal"])
    ...
    def l2_sample_size_calculator():
        ...
        total_samples = st.number_input(
            "Total number of samples: The total number of data points that third party will sample (typically between 100-1000). Range > 0", min_value=1, value=100)
        average_truth_score = st.slider(
            "Average truth score: The expected average truth score across all blocks (typically between 0.2-0.5). Ideally, should be based on some real data from the sector in question. Higher is worse (i.e. more mismatches between subordinate and 3P). 0 < Range < 1", min_value=0.0, max_value=1.0, value=0.5)
        variance_across_blocks = st.slider(
            "Variance across blocks: The expected standard deviation of mean truth score across blocks (typically between 0.1-0.5). Ideally, should be based on some real data from the sector in question. The higher this value, the easier it will be to correctly rank the blocks. Range > 0", min_value=0.0, max_value=1.0, value=0.1)
        variance_within_block = st.slider(
            "Variance within block: The expected standard deviation across subordinates within a block (typically between 0.1-0.5). Ideally, should be based on some real data from the sector in question. The higher this value, the more difficult it will be to correctly rank the blocks. Range > 0", min_value=0.0, max_value=1.0, value=0.1)
        level_test = st.selectbox(
            "Level of test: The aggregation level at which 3P will test and give reward/punishment.", ["Block", "District", "State"])
        n_subs_per_block = st.number_input(
            "Number of subordinates per block: The number of subordinates in a block. Range > 1", min_value=1, value=10)
        n_blocks_per_district = st.number_input(
            "Number of blocks per district: The number of blocks in a district. Range >= 1", min_value=1, value=5)
        n_district = st.number_input(
            "Number of districts: Number of districts. Range >= 1", min_value=1, value=1)
        n_simulations = st.number_input(
            "Number of simulations: By default, this should be set to 100. The number of times the algorithm will be run to estimate the number of samples required. Higher n_simulations will give a more accurate answer, but will take longer to run. Range > 1", min_value=1, value=100)
        min_sub_per_block = st.number_input(
            "Minimum subordinates per block: Minimum number of subordinates to be measured in each block. By default, this should be set to 1. 0 < Range < n_sub_per_block", min_value=1, value=1)
        ...
        if st.button("Calculate L2 Sample Size"):
            ...
            if response.status_code == 200:
                ...
                # Create plot
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=list(range(len(
                    result['value']['true_disc']))), y=result['value']['true_disc'], mode='lines', name='True Discrepancy'))
                fig.add_trace(go.Scatter(x=list(range(len(
                    result['value']['meas_disc']))), y=result['value']['meas_disc'], mode='markers', name='Measured Discrepancy'))
                fig.update_layout(
    ...
    def third_party_sampling_strategy():
        ...
        min_sub_per_block = st.number_input(
            "Minimum subordinates per block: Minimum number of subordinates to be measured in each block. By default, this should be set to 1. 0 < Range < n_sub_per_block", min_value=1, value=1)
        percent_blocks_plot = st.slider(
            "Percentage of blocks to plot", min_value=0.0, max_value=100.0, value=10.0)
        ...
        if st.button("Predict 3P Sampling Strategy"):
            ...
            response = requests.post(
                THIRD_PARTY_SAMPLING_ENDPOINT, json=input_data)
    ...
    pre_survey_option = st.sidebar.selectbox("Select Pre-survey Analysis", [
                                             "L1 Sample Size Calculator", "L2 Sample Size Calculator", "3P Sampling Strategy Predictor"])
...
def main():
    ...
    main_option = st.sidebar.selectbox(
        "Select Module", ["Pre-survey Analysis", "Administrative Data Quality"])
